/* questo codice sviluppato dal team Elettronico di Fastcharge è dedicato al funzionamento di un arduino DUE
...
int current;         // Current state of the button \\bottone premuto per il launch control
  ...
  */
  while (analogRead(SC) > SCthr && RTD && plaus1 && plaus2) {
    ...
    if (millis_held > 50) {
      ...
      if (secs_held >= 3) { //ho tenuto il pulsante per più di tre secondi
        digitalWrite(BUZZ, HIGH);
        delay(4000);
        digitalWrite(BUZZ, LOW);
        delay(2000);
        if (th1 > startLaunch ) {
          ...
          while (th1 > launch && launch < 4095) {    //fino a quando accelero sopra la rampa il launch control procede, se rilascio acceleratore esco dalla procedura   
            launch += launchInc;
            analogWrite(DAC1, map(launch, th1Low, th1Up, 0, 4095));
          }
        }
      }
  ...
  if (plaus1 && plaus2) {
    ...
    Serial.print("TH1: "); Serial.print(th1); Serial.print("   BRAKE: "); Serial.println(bk);
  }
