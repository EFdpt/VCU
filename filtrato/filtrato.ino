...
/* questo codice sviluppato da Valerio Dodet è dedicato al funzionamento di un arduino DUE
   ...
   DRIVE:stato 2, lo stato di guida sicura, accedibile tramite procedura RTD ma anche con lo
      ...
      durante la guida una media ponderata su 8 valori stabilizza i valori delle acquisizioni
      rallentando vistosamente la risposta dinamica all'input dei pedali.
      Per recuperare su questo
...
int i, j, res, pre1, pre2, preBk, attuale;
...
int RTDBK = 1130; //pressione freno per RTD
...
void setup() {
  ...
  posArrTh1 = posArrTh2 = posArrBk = 1;
...
void HVON() {
  ...
  for (int i = 0; i < lunghezza; i++) {
    posArrBk = acquisizioneCutBSE(posArrBk);
  }
  bk = mediaPond(ArrBk, posArrBk);
...
void DRIVE() {
  ...
  */
  while (/*analogRead(SC) > SCthr &&*/ RTD && plaus1 && plaus2) {
    ...
    posArrTh1 = acquisizioneCutTPS1(posArrTh1);
    posArrTh2 = acquisizioneCutTPS2(posArrTh2);
    posArrBk = acquisizioneCutBSE(posArrBk);

    th1 = mediaPond(ArrTh1, posArrTh1);
    th2 = mediaPond(ArrTh2, posArrTh2);
    bk = mediaPond(ArrBk, posArrBk);
    ...
    */
    ...
    //check rilascio forzato, diff tra filtro e pedale maggiore di 1/6 del valore mediato
    // check valori negativi da scrivere
    if ((th1 - analogRead(TPS1)) > (th1 / 4)) {

      //qui il pedale è stato rilasciato bruscamente, viene scritto il valore diretto del pedale
      for (j = 0; j != 6; j++) {
        posArrTh1 = acquisizioneCutTPS1(posArrTh1);
        posArrTh2 = acquisizioneCutTPS2(posArrTh2);
        posArrBk = acquisizioneCutBSE(posArrBk);
        attuale = analogRead(TPS1);
        Serial.print("TH1 attuale: "); Serial.println(attuale);
        analogWrite(DAC1, map(attuale, th1Low, dinamica2 + th1Low, 0, 2047));
      }
    }
    //feed forward, accelera la dinamica di risposta alla posizione del pedale
    th1 = (th1 + (analogRead(TPS1) * 3)) >> 2;
    ...
    if (plaus1 && plaus2) {
      ...
      if (th1 <= th1Low + dinamica2)
        analogWrite(DAC1, map(th1, th1Low, th1Low + dinamica2, 0, 2047));
      else
        analogWrite(DAC1, map(th1, th1Low + dinamica2, th1Up, 2048, 4095));
  ...
  */
  if (!plaus2) {
    while (mediaPond(ArrTh1, posArrTh1) > RunTH5) {
      posArrTh1 = acquisizioneCutTPS1(posArrTh1);
      posArrBk = acquisizioneCutBSE(posArrBk);
      Serial.println("   FRENO + ACCELERATORE !!!");
      //      //BYPASS BSPD
      //      if (mediaPond(bk,posArrBk) > 870) {
      //        digitalWrite(AIRGnd, LOW);
      //        digitalWrite(AIRcc, LOW);
      //        BSPD = 0;
      //        current_state = 0;
